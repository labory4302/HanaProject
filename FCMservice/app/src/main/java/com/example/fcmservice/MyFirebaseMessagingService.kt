...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if(remoteMessage.data.isNotEmpty()){
            Log.i("token 바디: ", remoteMessage.data["body"].toString())
            Log.i("token 타이틀: ", remoteMessage.data["title"].toString())
            sendNotification(remoteMessage)
        } else {
            Log.i("token 수신에러: ", "data가 비어있습니다. 메시지를 수신하지 못했습니다.")
            Log.i("token data값: ", remoteMessage.data.toString())
        }
    }
